...
Os dois primeiros itens são as maiores dívidas de honestidade em aberto — mais sérias que o resto desta lista, porque envolvem a regra #1 do domínio (nunca inventar/nunca fingir que leu), não só cobertura incompleta de layout.

- **Holerite escaneado não marca incerteza de OCR.** `extractors/holerite.py` nunca consulta `Palavra.confianca` em lugar nenhum — nem na montagem de rótulo, nem em `tokens.normalizar_dinheiro` (que só faz `regex` em cima do texto). Diferente do cartão de ponto, que degrada explicitamente para `"??:??"` abaixo de `OCR_CONF_THRESHOLD`. Na prática: num holerite escaneado (`payroll-04.pdf`), um dígito que o Tesseract leu errado com baixa confiança passa direto pro `value`/`label` como se tivesse certeza — sem `?`, sem log, sem sinal nenhum de que aquele caractere é duvidoso. Achado girando a configuração do Bloco 2 de hoje, não corrigido — é trabalho de calibração do mesmo tamanho do que os Dias 3/4 fizeram pro cartão de ponto (medir confiança real em exemplos, decidir limiar, testar degradação), e não cabia no orçamento de hoje.
- **`time-card-04.pdf` agora é recusado, não mais lido como se tivesse sido entendido.** Era a lacuna mais séria em aberto — corrigida hoje. Diagnóstico, medição e a correção (gate de "dias reconhecidos mas zero batidas em todo o documento") estão no `PROCESSO.md`, seção "Dia 6, Bloco 3".
- **`payroll-01.pdf` (ficha financeira) não é lido — e isso está correto.** Layout de 3 mini-tabelas lado a lado, título estilizado sem cabeçalho-âncora reconhecível e sem pares `Rótulo: Valor` — a gramática atual assume um bloco de colunas por página, e esse documento não tem isso. Levanta `LayoutDesconhecidoError` (`status: "erro"`, mensagem legível) em vez de inventar uma leitura. Precisaria de uma técnica genuinamente diferente (zonas por posição x dentro de cada bloco); decidi não implementar às pressas. Mesmo comportamento CERTO diante de layout não suportado que `time-card-04.pdf` agora também tem — recusar, não fingir.
- **Qual tipo ficou mais fraco**: cartão de ponto está mais maduro que holerite. Os dois têm um documento não suportado, mas o cartão de ponto tem calibração de OCR medida (binarização testada e descartada com dado, limiar de confiança justificado) e uma bateria maior de casos sintéticos (faixa colada, confiança baixa, ordem fora de sequência, área de totais) — e degrada por incerteza, o que o holerite ainda não faz (ver acima).
